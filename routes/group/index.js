...
const { tokenDecoder } = require("../../utils/jsonwebtoken");
...
GroupRouter.post("/requestjoin", (req, res, next) => {
  const data = req.body;
  const errs = [];
});
                  ...
                  .then((userExists) => {
                    if (userExists) {
                      group
                        .findById(data.group)
                        .then((groupDoc) => {
                          if (String(groupDoc._id) !== String(decoded._id)) {
                            errs.push(`Invalid admin token`);
                            errs.send(
                              response({
                                res: false,
                                data,
                                errs,
                                msg: "Group joining faled",
                              })
                            );
                          } else if (!groupDoc.members.includes(data.user)) {
                            group
                              .updateOne(
                                { _id: data.group },
                                {
